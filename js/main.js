(function ($) {
  ...
  jQuery(document).ready(function ($) {
    ...
    $("#downloadProfile").click(function () {
      ...
      link.href = url;
      link.download = `lotf-checklist-backup-${new Date().toISOString().split('T')[0]}.json`;
    ...
    $("#fileInput").change(function () {
      ...
      reader.onload = function (e) {
        try {
          ...
          if (!confirm("This will overwrite your existing profiles. Are you sure?")) {
            return;
          }

          // Restore the profiles
          profiles[profilesKey] = importedData.profiles;
          profiles.current = importedData.currentProfile || profiles.current;
          $.jStorage.set(profilesKey, profiles);
